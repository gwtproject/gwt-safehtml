 ...
 */
public class GwtSafeStylesHostedModeUtilsTest
    extends GWTTestCase {

  private static final String ERROR_MESSAGE_MISMATCH = "Expected error message does not match actual error message";
  ...
  public void testIsValidStyleName() {
    ...
    for (String s : GwtSafeStylesUtilsTest.VALID_STYLE_NAMES) {
      ...
      assertNull("'" + s + "' incorrectly reported as an invalid style name: " + error,
                 error);
    }
  ...
  public void testIsValidStyleValue() {
    ...
    for (String s : GwtSafeStylesUtilsTest.VALID_STYLE_VALUES) {
      String error = SafeStylesHostedModeUtils.isValidStyleValue(s);
      assertNull("'" + s + "' incorrectly reported as an invalid style value: " + error,
                 error);
    }
  ...
  public void testMaybeCheckValidStyleName() {
    ...
    try {
       ...
       * AssertionError).
       */
      assertEquals(ERROR_MESSAGE_MISMATCH,
                   expectedError,
                   e.getMessage());
      caught = true;
