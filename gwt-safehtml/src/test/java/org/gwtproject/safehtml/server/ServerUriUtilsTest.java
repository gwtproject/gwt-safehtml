 ...
 */
package org.gwtproject.safehtml.server;
...
import org.gwtproject.safehtml.shared.SafeUriHostedModeUtils;
...
public class ServerUriUtilsTest extends TestCase {
  ...
  static {
    ...
    badUris.add(new UriTestCaseSpec("  mailto:good@good.com", "  mailto", true));
  ...
  public static void testExtractScheme() {
    for (UriTestCaseSpec uriSpec : GOOD_URIS) {
      assertEquals(uriSpec.getScheme(), UriUtils.extractScheme(uriSpec.getUri()));
  ...
  /**
   * Encapsulates a URI and relevant attributes for use in tests of {@link
   * UriUtils#sanitizeUri(String)} and related methods.
   */
  private static class UriTestCaseSpec {
     ...
     * @param scheme the scheme that is expected to be parsed from {@code uri} by {@link
     *     UriUtils#extractScheme(String)}
     * @param expectUriParseException true if parsing {@code uri} into a {@link URI} object is
     *     expected to result in a {@link URISyntaxException}
     */
    public UriTestCaseSpec(String uri, String scheme, boolean expectUriParseException) {
      this.uri = uri;
      this.scheme = scheme;
      ...
      if (!expectUriParseException) {
        try {
          ...
          throw new IllegalStateException(
              "parsing \"" + uri + "\" resulted in " + "unexpected exception: " + e);
        }
      }
    }
