 ...
 *
 * <p>A chunk may correspond to a literal string or to a formal template parameter.
 *
 * <p>Parameter chunks have an attribute that refers to the index of the corresponding template
 * method parameter, as well as an attribute that represents the HTML context the template parameter
 * appeared in.
 */
final class ParsedHtmlTemplate {

  /**
   * A representation of the context of a point within a HTML document. A context consists of a type
   * (such as "plain text", "attribute"), as well as a HTML tag and attribute name where applicable.
   */
  static final class HtmlContext {
     ...
     *
     * @param type the {@link Type} of this context
     * @param tag the HTML tag this context corresponds to, if applicable; null otherwise
     * @param attribute the HTML attribute this context corresponds to, if applicable; null
     *     otherwise
     */
    public HtmlContext(Type type, String tag, String attribute) {
      Preconditions.checkArgument(
          (type != Type.TEXT) || ((tag == null) && (attribute == null)),
  ...
  static final class ParameterChunk implements TemplateChunk {
     ...
     * @param parameterIndex the index of the template parameter that this chunk refers to
     */
   ...
   *
   * <p>There are two kinds of chunks: Those representing literal strings and those representing
   * template parameters.
   */
   ...
   *
   * <p>If the currently last chunk of the parsed template is a literal chunk, the provided string
   * literal will be appended to that chunk. I.e., consecutive literal chunks are automatically
   * coalesced.
   *
   * @param literal the string to be added as a literal chunk
   */
  public void addLiteral(String literal) {
    if (chunks.isEmpty() || (chunks.getLast().getKind() != TemplateChunk.Kind.LITERAL)) {
