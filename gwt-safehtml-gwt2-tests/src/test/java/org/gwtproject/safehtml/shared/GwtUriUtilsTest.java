...
public class GwtUriUtilsTest extends GWTTestCase {
  ...
  public void testFromTrustedString() {
    ...
    if (GWT.isClient()) {
      assertEquals(
          GWT.getModuleBaseURL(), UriUtils.fromTrustedString(GWT.getModuleBaseURL()).asString());
      assertEquals(
  ...
  public void testUnsafeCastFromUntrustedString() {
    ...
    assertEquals(
        EMPTY_GIF_DATA_URL, UriUtils.unsafeCastFromUntrustedString(EMPTY_GIF_DATA_URL).asString());
    ...
    if (GWT.isClient()) {
      assertEquals(
          ...
          UriUtils.unsafeCastFromUntrustedString(GWT.getModuleBaseURL()).asString());
      assertEquals(
          GWT.getHostPageBaseURL(),
          UriUtils.unsafeCastFromUntrustedString(GWT.getHostPageBaseURL()).asString());
    }
  }
  ...
  public void testFromString() {
    ...
    assertEquals(
        UriUtils.sanitizeUri(EMPTY_GIF_DATA_URL),
        UriUtils.fromString(EMPTY_GIF_DATA_URL).asString());
    assertEquals(
        UriUtils.sanitizeUri(JAVASCRIPT_URL), UriUtils.fromString(JAVASCRIPT_URL).asString());
    if (GWT.isClient()) {
      assertEquals(GWT.getModuleBaseURL(), UriUtils.fromString(GWT.getModuleBaseURL()).asString());
      assertEquals(
          GWT.getHostPageBaseURL(), UriUtils.fromString(GWT.getHostPageBaseURL()).asString());
    }
  }

  @Override
  public String getModuleName() {
    return "org.gwtproject.safehtml.SafeHtmlTestsModule";
  }
}
