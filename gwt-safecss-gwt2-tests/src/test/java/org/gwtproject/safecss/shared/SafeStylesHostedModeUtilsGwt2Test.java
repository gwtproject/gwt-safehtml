...
public class SafeStylesHostedModeUtilsGwt2Test extends GWTTestCase {
  ...
  public void testIsValidStyleName() {
    ...
    // Valid names.
    for (String s : SafeStylesUtilsGwt2Test.VALID_STYLE_NAMES) {
    ...
    // Invalid names.
    for (String s : SafeStylesUtilsGwt2Test.INVALID_STYLE_NAMES) {
      assertNotNull(
  ...
  public void testIsValidStyleValue() {
    ...
    // Valid values.
    for (String s : SafeStylesUtilsGwt2Test.VALID_STYLE_VALUES) {
      String error = SafeStylesHostedModeUtils.isValidStyleValue(s);
      assertNull("'" + s + "' incorrectly reported as an invalid style value: " + error, error);
    }

    // Invalid values.
    for (String s : SafeStylesUtilsGwt2Test.INVALID_STYLE_VALUES) {
      assertNotNull(
  ...
  public void testMaybeCheckValidStyleName() {
    ...
    String expectedError =
        SafeStylesHostedModeUtils.isValidStyleName(SafeStylesUtilsGwt2Test.INVALID_STYLE_NAME);
    assertNotNull(expectedError);
    boolean caught = false;
    try {
      SafeStylesHostedModeUtils.maybeCheckValidStyleName(
          SafeStylesUtilsGwt2Test.INVALID_STYLE_NAME);
  ...
  public void testMaybeCheckValidStyleValue() {
    ...
    String expectedError =
        SafeStylesHostedModeUtils.isValidStyleValue(SafeStylesUtilsGwt2Test.INVALID_STYLE_VALUE);
    assertNotNull(expectedError);
    boolean caught = false;
    try {
      SafeStylesHostedModeUtils.maybeCheckValidStyleValue(
          SafeStylesUtilsGwt2Test.INVALID_STYLE_VALUE);
    } catch (IllegalArgumentException e) {
      /*
